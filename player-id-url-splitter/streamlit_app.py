...
def main():
    ...
    st.markdown("**Step 1: Open Developer Tools:** Press **Ctrl+Shift+I** (Windows/Linux) or **Cmd+Option+I** (Mac) in Chrome. For other browsers like Firefox or Edge, the same shortcuts apply.")
    st.markdown("[Learn more about Chrome DevTools](https://developer.chrome.com/docs/devtools/)")
    st.markdown("**Step 2: Access the Console:** Click the **Console** tab within DevTools.")
    st.markdown("**Step 3: Copy the Export URL:** In the PlayerTek dashboard, navigate to **ExportService** (URL containing `playertek.com/webinterface/ExportService`), then copy the full URL from the address bar.")
    st.markdown("**Step 4: Paste & Export:** Paste the copied URL into the field above and click **🚀 Export CSVs** to split and download your CSV files.")
